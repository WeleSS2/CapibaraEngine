...
private:
	...
	STEAM_GAMESERVER_CALLBACK( NetworkServer, OnSteamServersConnected, SteamServersConnected_t );

	// Tells us when there was a failure to connect to Steam
	STEAM_GAMESERVER_CALLBACK( NetworkServer, OnSteamServersConnectFailure, SteamServerConnectFailure_t );

	// Tells us when we have been logged out of Steam
	STEAM_GAMESERVER_CALLBACK( NetworkServer, OnSteamServersDisconnected, SteamServersDisconnected_t );

	// Tells us that Steam has set our security policy (VAC on or off)
	STEAM_GAMESERVER_CALLBACK( NetworkServer, OnPolicyResponse, GSPolicyResponse_t );
	...
	STEAM_GAMESERVER_CALLBACK( NetworkServer, OnValidateAuthTicketResponse, ValidateAuthTicketResponse_t );

	// client connection state
	// All connection changes are handled through this callback
	STEAM_GAMESERVER_CALLBACK(NetworkServer, OnNetConnectionStatusChanged, SteamNetConnectionStatusChangedCallback_t);
